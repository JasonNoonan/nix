...
**--code:** Remove unused imports, aliases & dead code | Clean Logger.info|warning|error, console.logs, debug & dbg, & IO.inspect code | Remove commented blocks | Fix style inconsistencies | Remove TODO>30 days

**--files:** Remove build artifacts & temp files | Clean deps folder if corrupted | Remove logs & cache dirs | Clean test outputs | Remove OS files (.DS_Store, thumbs.db)

**--deps:** Remove unused deps from mix.exs | Alphabetize deps in mix.exs | Check outdated packages
